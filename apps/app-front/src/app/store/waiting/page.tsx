...
const WaitingInfo = () => (
  <div className="text-center my-8">
    <h2 className="text-base sm:text-lg md:text-xl mb-3">
      휴대폰 번호를 입력하시면 카카오톡으로
    </h2>
...
const StoreName = () => (
  <div className="mt-10">
    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-center mb-6">
      강고기 홍대점
    </h1>
  </div>
);
...
// 숫자 패드 컴포넌트
const NumberPad = ({ onNumberClick, onReset, onBackspace }: { onNumberClick: (num: string) => void, onReset: () => void, onBackspace: () => void }) => (
...
const ActionButtons = ({ isPhoneNumberComplete }: { isPhoneNumberComplete: boolean }) => (
  <div className="flex justify-between mt-auto">
    <button 
      className="w-[48%] h-14 bg-gray-100 text-gray-800 rounded-md hover:bg-gray-200"
    >
      웨이팅 목록
    </button>
    <button 
