...
export default function PortfoliosPage() {
  ...
  const { data, isLoading } = useSWR(['screens', filters], () =>
    screenRepository.getList({ search: filters.search }),
  );
  ...
  const columns = [
    { id: 'applicationName', label: '앱명', minWidth: 150 },
    { id: 'name', label: '스크린명', minWidth: 150 },
  ...
  const moblieColumns = [
    { id: 'applicationName', label: '앱명', minWidth: 100 },
    { id: 'name', label: '스크린명', minWidth: 100 },
    { id: 'url', label: 'url', minWidth: 100 },
    { id: 'width', label: 'width', minWidth: 50 },
    { id: 'height', label: 'height', minWidth: 50 },
  ];
  ...
  const Actions = () => (
    ...
    >
      ...
      <Button
        variant='contained'
        startIcon={<AddIcon />}
        onClick={() => router.push('/screens/create')}
        sx={{ p: isMoblie ? 0 : 1 }}
      >
        {isMoblie ? '등록' : '스크린 등록'}
      </Button>
