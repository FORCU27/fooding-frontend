...
const menu = [
  ...
  },
  {
    label: '가게',
    icon: <Store />,
    sub: [{ label: '가게 관리', path: '/stores', icon: <Store /> }],
  },
  {
    label: '운영',
    icon: <Monitor />,
                      ...
                      <Link href={sub.path} key={sub.label} style={{ textDecoration: 'none' }}>
                        <ListItemButton
                          sx={{
                            ...
                            background:
                              active === sub.label ? 'rgba(59, 130, 246, 0.15)' : 'inherit',
                            '&:hover': {
                              color: '#1e40af',
                              background: 'rgba(59, 130, 246, 0.1)',
                            },
...
const AdminLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  if (!user || hideSidebarPath.includes(pathname)) {
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <Box sx={{ flexGrow: 1, background: '#ffffff', minHeight: '100vh' }}>
        <AppBar
          position='static'
          elevation={1}
          sx={{ background: '#ffffff', color: '#1e293b', borderBottom: '1px solid #e2e8f0' }}
        >
          <Toolbar>
